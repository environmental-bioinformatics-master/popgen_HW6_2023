...
# Minor modifications to *.snp and *.ind output file parsing / formatting by C. Tepolt, Nov 2023
    ...
    for line in vcf:
        if line[:2]=="##":				  # Comment line
            ...
            if len(bits[3])!=1 or len(bits[4])!=1:
                ...
                if bits[2]==".":
                    bits[2]=bits[0]+"_"+bits[1]
                snp.write("    ".join([bits[2], "1", "0.0", bits[2], bits[3], bits[4]])+"\n")
                geno_string=""
                if options["ref"]:
                    geno_string="2"
                for gt in bits[9:]:
